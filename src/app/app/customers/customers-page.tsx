...
export default function CustomersPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    let q = supabase().from("customers").select("*, work_orders(count)").order("name");
  ...
  async function openHistory(c: Customer) {
    ...
    const { data } = await supabase()
      .from("work_orders")
      .select("id, services, status, invoice_status, total_price, mechanic_pay, created_at, completed_at, notes, mechanic_name, vehicle:vehicles(plate, make, model, year)")
      .eq("customer_id", c.id)
  ...
  const statusColors: Record<string, string> = {
    queued: "#6b7280", in_progress: "#f59e0b", done: "#10b981", archived: "#374151",
  };
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <Table headers={[t("c.name"), t("c.phone"), t("c.email"), t("cust.company"), t("th.type"), t("c.visits"), ""]}>
          {customers.map((c) => (
            <tr key={c.id} onClick={() => { setEditing(c); setShowForm(true); }} style={{ cursor: "pointer" }}>
              ...
              <td style={tdStyle}>{(c as any).work_orders?.[0]?.count || 0}</td>
              <td style={{ ...tdStyle, display: "flex", gap: 4 }}>
                <Btn variant="ghost" style={{ fontSize: 11, padding: "4px 8px" }} onClick={(e: any) => { e.stopPropagation(); openHistory(c); }}>📋</Btn>
      ...
      <Modal open={showHistory} onClose={() => setShowHistory(false)}
        title={historyCustomer ? `${historyCustomer.name}` : "Istorija"} width={720}>
        {historyCustomer && (
          <div>
            {/* Customer info header */}
            <div style={{
              display: "flex", gap: 20, padding: "12px 16px", background: "var(--bg)",
              borderRadius: 8, marginBottom: 16, flexWrap: "wrap",
            }}>
              <div style={{ fontSize: 12 }}>
                <span style={{ color: "var(--muted)" }}>Tel: </span>
                <span style={{ fontWeight: 600 }}>{historyCustomer.phone || "—"}</span>
              </div>
              <div style={{ fontSize: 12 }}>
                <span style={{ color: "var(--muted)" }}>El. p.: </span>
                <span style={{ fontWeight: 600 }}>{historyCustomer.email || "—"}</span>
              </div>
              <div style={{ fontSize: 12 }}>
                <span style={{ color: "var(--muted)" }}>Įmonė: </span>
                <span style={{ fontWeight: 600 }}>{historyCustomer.company || "—"}</span>
              </div>
              <div style={{ fontSize: 12 }}>
                <span style={{ color: "var(--muted)" }}>Tipas: </span>
                <Badge color={historyCustomer.source === "naujas" ? "var(--accent-2)" : "var(--accent)"}>
                  {historyCustomer.source === "naujas" ? t("c.typeNew") : historyCustomer.source === "senas" ? t("c.typeOld") : t("c.partner")}
                </Badge>
              </div>
              {historyCustomer.notes && (
                <div style={{ fontSize: 12, width: "100%" }}>
                  <span style={{ color: "var(--muted)" }}>Pastabos: </span>{historyCustomer.notes}
                </div>
              )}
            </div>
            ...
            {!historyLoading && historyOrders.length > 0 && (
              <div style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
                <div style={{ padding: "8px 14px", background: "var(--bg)", borderRadius: 8, border: "1px solid var(--border)" }}>
                  <div style={{ fontSize: 10, color: "var(--muted)", textTransform: "uppercase" }}>Vizitai</div>
                  <div style={{ fontSize: 20, fontWeight: 700 }}>{historyOrders.length}</div>
                </div>
                <div style={{ padding: "8px 14px", background: "var(--bg)", borderRadius: 8, border: "1px solid var(--border)" }}>
                  <div style={{ fontSize: 10, color: "var(--muted)", textTransform: "uppercase" }}>Bendra suma</div>
                  <div style={{ fontSize: 20, fontWeight: 700 }}>€{historyOrders.reduce((s, o) => s + (o.total_price || 0), 0).toFixed(0)}</div>
                </div>
                <div style={{ padding: "8px 14px", background: "var(--bg)", borderRadius: 8, border: "1px solid var(--border)" }}>
                  <div style={{ fontSize: 10, color: "var(--muted)", textTransform: "uppercase" }}>Pirmas vizitas</div>
                  <div style={{ fontSize: 14, fontWeight: 600 }}>{historyOrders[historyOrders.length - 1]?.created_at?.slice(0, 10) || "—"}</div>
                </div>
                <div style={{ padding: "8px 14px", background: "var(--bg)", borderRadius: 8, border: "1px solid var(--border)" }}>
                  <div style={{ fontSize: 10, color: "var(--muted)", textTransform: "uppercase" }}>Paskutinis</div>
                  <div style={{ fontSize: 14, fontWeight: 600 }}>{historyOrders[0]?.created_at?.slice(0, 10) || "—"}</div>
                </div>
              </div>
            )}
            ...
            {/* Orders list */}
            {historyLoading ? (
              <div style={{ padding: 30, textAlign: "center", color: "var(--muted)" }}>{t("c.loading")}</div>
            ) : historyOrders.length === 0 ? (
              <div style={{ padding: 30, textAlign: "center", color: "var(--muted)" }}>Nėra užsakymų</div>
            ) : (
                ...
                {historyOrders.map((wo, i) => (
                  <div key={wo.id} style={{
                    ...
                    <div style={{
                      ...
                      background: statusColors[wo.status] || "#6b7280",
                    }} />
                    ...
                    <div style={{ width: 140, flexShrink: 0 }}>
                      <div style={{ fontSize: 12, fontWeight: 700 }}>{wo.vehicle?.plate || "—"}</div>
                      <div style={{ fontSize: 10, color: "var(--muted)" }}>
                        {[wo.vehicle?.make, wo.vehicle?.model, wo.vehicle?.year].filter(Boolean).join(" ")}
                      </div>
                    </div>

                    {/* Services */}
                    <div style={{ flex: 1, display: "flex", gap: 4, flexWrap: "wrap" }}>
                      {(wo.services || []).map((s: string) => (
                        <span key={s} style={{
                    ...
                    {/* Price */}
                    <div style={{ width: 70, textAlign: "right", fontSize: 13, fontWeight: 700, flexShrink: 0 }}>
                      {wo.total_price ? `€${wo.total_price}` : "—"}
                    </div>

                    {/* Status */}
                    <div style={{ width: 80, flexShrink: 0 }}>
                      <Badge color={statusColors[wo.status] || "#6b7280"}>
                        {wo.status === "queued" ? "Eilėje" : wo.status === "in_progress" ? "Vykdoma" : wo.status === "done" ? "Atlikta" : wo.status === "archived" ? "Archyvas" : wo.status}
                      </Badge>
                    </div>
                  </div>
                ))}
            ...
            <Row gap={10} style={{ justifyContent: "flex-end", marginTop: 16 }}>
              <Btn variant="ghost" onClick={() => {
                ...
                setEditing(historyCustomer);
                setShowForm(true);
              }}>✎ Redaguoti klientą</Btn>
              <Btn variant="ghost" onClick={() => setShowHistory(false)}>{t("c.close")}</Btn>
            </Row>
          </div>
        )}
      </Modal>
