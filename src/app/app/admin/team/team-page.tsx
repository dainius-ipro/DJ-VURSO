...
function TeamForm({ open, onClose, onSaved, member, tenantId }: {
  ...
  useEffect(() => {
    if (member) {
      ...
      setHourlyRate(""); setSalary(""); setIsActive(true); setSendInviteOnCreate(true); setShowRoleChange(false);
    }
  }, [member, open]);
  ...
  return (
    <Modal open={open} onClose={onClose} title={isEdit ? t("team.editMember") : t("team.newMember")} width={500}>
      ...
      <Grid cols={3} gap={14}>
        {isEdit && member?.role ? (
          <div>
            <label style={{ fontSize: 12, fontWeight: 600, color: "var(--muted)", display: "block", marginBottom: 4 }}>{t("team.role")}</label>
            <div style={{ padding: "9px 12px", background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 8, fontSize: 13, display: "flex", alignItems: "center", gap: 8 }}>
              <Badge color={ROLE_COLORS[role]}>{ROLE_LABELS[role]}</Badge>
              <button onClick={() => setShowRoleChange(!showRoleChange)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--muted)", fontSize: 11, marginLeft: "auto" }}>
                {showRoleChange ? "✕" : "✎"}
              </button>
            </div>
            {showRoleChange && (
              <Select label="" options={roleOpts} value={role} onChange={(e) => setRole(e.target.value as UserRole)}
                style={{ marginTop: 6 }} />
            )}
          </div>
        ) : (
