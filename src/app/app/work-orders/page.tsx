  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <Table headers={[t("c.date"), `${t("wo.vehicle")} / ${t("cust.title")}`, t("c.phone"), t("wo.services"), t("wo.worker"), t("c.price"), t("wo.parts"), t("c.status"), ""]}>
          {orders.map(o => (
            <tr key={o.id} style={{ cursor: "pointer", background: ROW_BG[o.status] || "transparent", transition: "background 0.15s" }} onClick={() => { setEditing(o); setShowForm(true); }}>
              <td style={{ ...tdStyle, padding: "4px 8px" }}>
                <div style={{ fontWeight: 600, fontSize: 12 }}>{o.order_date}</div>
                <div style={{ fontSize: 10, color: "var(--muted)" }}>{o.day_of_week}</div>
                {o.invoice_number && <div style={{ fontSize: 10, color: "var(--accent)" }}>📄 {o.invoice_number}</div>}
              </td>
              <td style={{ ...tdStyle, padding: "4px 8px" }}>
                {o.vehicle ? (
                  <>
                    <div style={{ fontWeight: 700, fontSize: 12 }}>{o.vehicle.plate}</div>
                    <div style={{ fontSize: 11, color: "var(--muted)" }}>
                      {o.vehicle.make} {o.vehicle.model}{o.vehicle.cc ? ` • ${o.vehicle.cc}cc` : ""}{o.vehicle.kw ? ` • ${o.vehicle.kw}kW` : ""}
                    </div>
                  </>
                ) : <span style={{ color: "var(--muted)" }}>—</span>}
                {o.customer && <div style={{ fontSize: 11, color: "var(--accent)", fontWeight: 600 }}>{o.customer.name}</div>}
              </td>
              <td style={{ ...tdStyle, padding: "4px 8px", fontSize: 12 }}>
                {o.customer?.phone || "—"}
              </td>
              <td style={{ ...tdStyle, padding: "4px 8px" }}>
              ...
              <td style={{ ...tdStyle, padding: "4px 8px", fontSize: 12 }}>
                ...
                {(o as any).executor_name && <div style={{ fontSize: 10, color: "var(--muted)" }}>🏢 {(o as any).executor_name}</div>}
              </td>
              <td style={{ ...tdStyle, padding: "4px 8px" }}>
                <div style={{ fontWeight: 700, fontSize: 13 }}>€{(o.service_price || 0).toFixed(0)}</div>
                {isOwner && o.repair_cost > 0 && <div style={{ fontSize: 10, color: "var(--muted)" }}>Darbas: €{o.repair_cost.toFixed(0)}</div>}
              </td>
              <td style={{ ...tdStyle, padding: "4px 8px" }}>
              ...
              <td style={{ ...tdStyle, padding: "4px 8px" }}>
                ...
                {o.is_paid && <div style={{ marginTop: 2 }}><Badge color="var(--success)">💳</Badge></div>}
              </td>
              <td style={{ ...tdStyle, padding: "4px 6px" }}>
                <Btn variant="ghost" style={{ fontSize: 11, padding: "4px 6px" }}
                  onClick={(e: any) => { e.stopPropagation(); setEditing(o); setShowForm(true); }}>✎</Btn>
              </td>
            </tr>
          ))}
          {orders.length === 0 && (
            <tr><td colSpan={9} style={{ ...tdStyle, textAlign: "center", color: "var(--muted)", padding: 30 }}>{t("c.noOrders")}</td></tr>
          )}
        </Table>
      )}
    ...
    <Modal open={open} onClose={onClose} title={isEdit ? t("wo.editOrder") : t("wo.new")} width={760}>
        ...
        <>
          ...
          <Grid cols={2} gap={14}>
            ...
            <SearchSelect
              ...
              options={customers.map(c => ({
                value: c.id,
                label: c.phone ? `${c.phone} — ${c.name}` : c.name,
                sub: [c.email, c.company, c.name, c.phone].filter(Boolean).join(" "),
              }))}
          ...
          <div style={{ marginTop: 16, padding: 14, background: "var(--bg)", borderRadius: 10, border: "1px solid var(--border)" }}>
            ...
            <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 4 }}>
              = Darbas €{(calcRepair + calcProg).toFixed(0)} + Detalės €{calcPartsRev.toFixed(0)}
            </div>
      ...
      {tab === "finance" && (
        <>
          ...
          <Grid cols={2} gap={14}>
            <Input label="Darbo kaina €" type="number" value={repairCost} onChange={e => setRepairCost(e.target.value)} />
            <div>
